...
export default function CallScreen({ route }) {
    ...
    useEffect(() => {
        const { roomId, token, displayName } = route.params
        setState(prev => { return { ...prev, roomId, token, displayName } })
    }, [route]);
    ...
    const startCall = async (roomId) => {
        ...
        const roomRef = await firestore().collection('videorooms').doc(roomId);
        ...
        roomRef.onSnapshot(async snapshot => {
            ...
            if ((!!localPC.current && !localPC.current.currentRemoteDescription) && !!data && !!data.answer) {
                const rtcSessionDescription = new RTCSessionDescription(data.answer);
                await localPC.current.setRemoteDescription(rtcSessionDescription);
            }
        });
        ...
        <View style={{ flex: 1, backgroundColor: '#eff1e4', }} >
            ...
            <View style={styles.callButtons} >
                ...
                {localStream &&
                    ...
                    >
                        <FontAwesome5Icon
                            name={`${!isVideo ? 'video' : 'video-slash'}`}
                            size={20} color={'#000'}
                        />
                    </TouchableOpacity>
                }
