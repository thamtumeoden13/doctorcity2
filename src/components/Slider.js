...
export default class Slider extends PureComponent {
  ...
  _onHandlerStateChange = (event) => {
    if (event.nativeEvent.state === State.BEGAN) {
      ...
      // This synthetic event is reused for performance reasons, so save it first
      var translationX = this.props.vertical
  ...
  render() {
    ...
    if (vertical) {
      ...
      translate = Animated.multiply(
        Animated.add(translate, -containerSize.width),
        -1,
      );
    }
    ...
    return (
      <View
        ...
        <PanGestureHandler
          onGestureEvent={
            this.props.disabled ? undefined : this._onGestureEvent
          }
          onHandlerStateChange={
            this.props.disabled ? undefined : this._onHandlerStateChange
          }>
          <Animated.View
            renderToHardwareTextureAndroid={true}
  ...
  _handlePanResponderGrant = (x: number) => {
    ...
    this._translateX.setOffset(
      this.props.vertical
        ? this.state.containerSize.width - this._lastOffsetX
        : this._lastOffsetX,
    );
    this._translateX.setValue(0);
    ...
    if (this._containerSize && this._trackSize && this._thumbSize) {
      this.setState(
        ...
        () => {
          ...
          this._translateX.setOffset(
            this.props.vertical
              ? this.state.containerSize.width - offset
              : offset,
          );
          this._translateX.setValue(0);
